...
class LoginPage:
    ...
    def get_url(self, url):
        configName.driver.get(url)

    def check_title(self, title):
        configName.WebDriverWait(configName.driver, 30).until(configName.EC.text_to_be_present_in_element(self.title, title))
        title_element = configName.WebDriverWait(configName.driver, 30).until(configName.EC.presence_of_element_located(self.title))
    ...
    def click_login(self):
        login_element = configName.WebDriverWait(configName.driver, 30).until(configName.EC.presence_of_element_located(self.login_btn))
        Utils.highLight(login_element)
        login_element.click()

    def check_login(self, dashboard_title):
        configName.WebDriverWait(configName.driver, 30).until(configName.EC.text_to_be_present_in_element(self.dashboard, dashboard_title))
        dashboard_element = configName.WebDriverWait(configName.driver, 30).until(configName.EC.presence_of_element_located(self.dashboard))
